...
export type { RemoteChannelBootSeat } from '../remote-methods.ts'
export { REMOTE_CHANNEL_BOOT_GLOBAL } from '../remote-methods.ts'
...
/**
 * The cookieless device credential: this tab's sessionStorage first, then the
 * localStorage copy the /pair-app capture script also writes (a restored tab,
 * or one whose sessionStorage was reclaimed, still presents the credential —
 * which is the only one the cookieless flow ever has).
 * @param window - the browser window (or a test double).
 * @returns the stored device id, or null.
 */
export function readStoredDevice(window: Pick<ChannelWindow, 'sessionStorage' | 'localStorage'>): string | null {
  try {
    const session = window.sessionStorage?.getItem(RULES.deviceKey) ?? null
    if (session !== null) return session
  } catch {
    // Storage disabled: fall through to the durable copy.
  }
  try {
    return window.localStorage?.getItem(RULES.deviceKey) ?? null
  } catch {
    return null
  }
}
...
export function installRemoteChannel(window: ChannelWindow, options: RemoteChannelOptions = {}): () => void {
  ...
  const device = readStoredDevice(window)
