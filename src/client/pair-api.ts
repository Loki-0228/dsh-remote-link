...
export interface IssueResult {
  ...
  token: string
  expiresAt: number
  /** Every constructible LAN base address, ranked (entry 0 is the reachable one). */
  lanAddresses: string[]
  /** The subset that looks like a virtual/tunnel adapter, for labelling. */
...
/**
 * Whether the LAN-bind facts are readable from this page. The endpoint is a
 * loopback-only control surface (changing the bind is a host action), so a
 * phone or another computer always gets a 403 — the panel uses this to explain
 * the situation instead of rendering a dead control.
 * @param hostname - the page hostname (defaults to the live one).
 * @returns true when this page can read and change the LAN bind.
 */
export function canControlLanBind(hostname: string = window.location.hostname): boolean {
