/**
 * The inner browser credential for the remote-link channel. The proxied /api
 * re-issues requests to 127.0.0.1, where the connection plugin's route
 * enforces the harness browser-auth cookie — an authority-bound signed cookie
 * with no loopback exemption on this harness line, so a cookie minted for the
 ...
 * device's first navigation performs) and attaches the resulting cookie to
 * inner requests. The credential is only ever exercised behind the pairing
 * gate, which runs before this module is consulted.
 * @module dsh-remote-link/inner-auth
 */
 ...
 *   (the connection service's authenticatedUrl), or undefined when the
 *   connection service or port is unavailable.
