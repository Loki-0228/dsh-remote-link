 ...
 * cordis-facing surfaces (routes, the remote channel) live next door.
 *
 * Security invariants:
...
/**
 * One tunnel status frame (the managed SakuraFrp tunnel; undefined when the
 * feature is off).
 */
...
export interface PairingSnapshot {
  ...
  publicUrl?: string
  /** Managed-tunnel status, while the tunnel feature is active. */
  tunnel?: TunnelStatus
...
export class UnknownLanAddressError extends Error {
  ...
  constructor(address: string) {
    super(`dsh-remote-link: unknown LAN address ${JSON.stringify(address)}`)
...
export class PairingService {
  ...
  /** Public (tunneled) base URL, e.g. a SakuraFrp tunnel address. */
  ...
  private persist(): void {
    ...
    try {
      ...
      console.error('dsh-remote-link: failed to persist paired devices', error)
    }
  }
  ...
  /** Set or clear the managed-tunnel status frame (undefined when the feature is off). */
  ...
  issue(workspaceId?: string, address?: string): { token: string; expiresAt: number } {
    if (this.lanBases.size === 0 && this.publicBase === undefined) {
      throw new Error('dsh-remote-link: pairing requires a reachable bind (--host 0.0.0.0 or publicBaseUrl)')
    }
  ...
  private notify(): void {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        console.error('dsh-remote-link: pairing state listener failed', error)
      }
    }
  }
}
