...
import { makeViewerRoutes, type ViewerContext, type ViewerRoot } from './file-viewer.ts'
import { REMOTE_DEVICE_STORAGE_KEY } from './remote-methods.ts'
import { lanCandidates } from './lan.ts'
...
function logEntryRefusal(request: IncomingMessage, path: string, reason: string): void {
  ...
  console.log(`dsh-remote-link: refused ${path} from host ${request.headers.host ?? '-'} (${reason}; ${markers})`)
}
 ...
 * script runs — the same key the parse-time boot patch and the channel's
 * browser half read (see {@link REMOTE_DEVICE_STORAGE_KEY}). The replaceState
 * to '/' keeps the credential out of the address bar and leaves the SPA at its
 * canonical root path. The reopen service worker is registered in the same
 * breath: later navigations to '/' (history, bookmark, tab restore) must not
 * fall through to the harness index gate, which the cookieless flow can never
 * satisfy.
 */
export const APP_DEVICE_STORAGE_KEY = REMOTE_DEVICE_STORAGE_KEY
...
export const PAIR_PATHS = {
  ...
  /**
   * The root document, served to an already-paired device (the capture script
   * rewrites the address bar here, so reload/bookmark/tab-restore land on it).
   */
  root: '/',
} as const
...
export interface PairRoutesDeps {
  ...
  /**
   * The official index document to serve on the /pair-app landing and at the
   * root for a paired device. The plugin fetches it from the inner loopback
   * with its own credential and patches it with the device-capture script, so
   * a paired device loads the app shell WITHOUT passing the harness index gate
   * - the mobile flow then needs no browser cookie at all (the channel gate
   * accepts the cookieless header/query credential). The device's own
   * authority rides along as a second-chance subject (see fetchAppShell in
   * src/index.ts). Undefined drops the route (tests).
   */
  indexDocument?: (deviceId: string, requesterAuthority?: string) => Promise<string | undefined>
  /** Official browser-cookie and launch-token owner; false means response handled. */
  ...
  /**
   * The read-only file viewer (its own route paths, sharing this family's
   * LAN fence). Undefined drops the whole viewer (tests, and deployments that
   * do not want the surface).
   */
  viewer?: { roots: (ctx: ViewerContext) => Promise<ViewerRoot[]> | ViewerRoot[]; ctx: ViewerContext }
}
...
export function makeRoutes(deps: PairRoutesDeps): WebRoute[] {
  ...
  const handleIssue = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      if (base === undefined) throw new Error('dsh-remote-link: base unavailable')
      writeJson(res, 200, {
        ok: true,
        ...
        // advertised network without a second round trip. The list is ranked:
        // entry 0 is the address a LAN device can actually reach.
        lanAddresses: service.lanAddresses,
        // The subset that looks like a virtual/tunnel adapter (WSL, Hyper-V, a
        // VPN client). They are still offered — a VPN peer can be a legitimate
        // client — but the panel marks them, since choosing one is the usual
        // reason a scanned QR never connects.
        lanVirtualAddresses: lanCandidates().filter(candidate => candidate.virtual).map(candidate => candidate.address),
  ...
  /**
   * Serve the official shell to one paired device: the index document with the
   * device-capture script spliced in, plus the device cookie (a browser that
   * dropped it mid-session, e.g. a WebView reclaiming storage, re-arms here).
   * The shell carries no data, so the device credential is the whole check.
   * @param req - the phone-facing request (already past the LAN fence).
   * @param res - the response owning the shell.
   * @param id - the live paired device id.
   */
  const serveAppShell = async (req: IncomingMessage, res: ServerResponse, id: string): Promise<void> => {
    const host = req.headers.host
    const html = await deps.indexDocument?.(id, typeof host === 'string' ? host : undefined).catch(() => undefined)
    if (html === undefined) {
  ...
  const handleAppPage = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    await serveAppShell(req, res, id)
  }

  /**
   * The official shell AT THE ROOT for an already-paired device.
   *
   * The /pair-app capture script rewrites the address bar to `/` (the
   * credential must not sit in the URL), and that is where a bookmark, a tab
   * restore or a plain reload comes back to. Without this seat those
   * navigations land on the harness's index fallback, which authenticates
   * with the browser-auth cookie only: a paired phone never holds one (the
   * capture script's whole point is that the device credential needs no
   * cookie), so the reload answered 401 and the only way back in was to scan
   * — or paste — the pairing link again, which also re-armed a token that
   * should have died with its TTL. The seat serves the same shell /pair-app
   * does, so a reload is just a reload.
   *
   * Exact routes own the response, including query strings: writing a 404
   * NEVER resumes the fallback. Launch tokens and desktop cookies therefore
   * go explicitly through the official Connection authorization API.
   */
  const handleRootApp = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const id = deviceIdOf(req)
    if (id !== undefined) {
      if (req.method === 'GET') await serveAppShell(req, res, id)
      else res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' }).end()
      return
    }
    // Not ours: write the fallback owner's refusal so an unpaired visitor sees
    // exactly what the harness seat answers today.
  ...
  /**
   * The live paired device a phone-facing request presents: the app-page
   * `?device=` credential first (a landing that arrives with it has not
   * necessarily stored the cookie yet), then the pairing cookie. Either
   * refreshes the session's presence.
   * @param req - the incoming request.
   * @returns the live device id, or undefined.
   */
  const deviceIdOf = (req: IncomingMessage): string | undefined => {
    const url = new URL(req.url ?? '/', 'http://pair.invalid')
    const query = url.searchParams.get('device') ?? ''
    if (query !== '' && service.touchDevice(query)) return query
    const cookie = readCookie(req.headers.cookie, service.config.cookieName)
    if (cookie !== undefined && service.touchDevice(cookie)) return cookie
    return undefined
  }
  ...
  /**
   * The root seat delegates launch-token and desktop-cookie checks to
   * Connection, and serves paired remote devices through their own gate.
   */
  const handleRootRequest = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    const url = new URL(req.url ?? '/', 'http://pair.invalid')
    if (req.method !== 'GET' && req.method !== 'HEAD') {
      res.writeHead(405).end()
      return
    }
    if (url.searchParams.has('token') || isLoopbackClient(req)) {
      if (deps.authorizeIndex === undefined) {
        res.writeHead(503, { 'cache-control': 'no-store' }).end('browser authentication unavailable')
        return
      }
      // This both redeems /?token= and verifies the persistent desktop cookie.
      // Never substitute a remote device identity for the desktop credential.
      if (!deps.authorizeIndex(req, res)) return
      const html = await deps.indexDocument?.('', req.headers.host).catch(() => undefined)
      res.writeHead(html === undefined ? 502 : 200, {
        'content-type': 'text/html; charset=utf-8',
        'cache-control': 'no-store',
        'referrer-policy': 'no-referrer',
      })
      res.end(req.method === 'HEAD' ? undefined : (html ?? 'app unavailable'))
      return
    }
    await handleRootApp(req, res)
  }
  ...
  if (deps.indexDocument !== undefined) {
    ...
    // Root owns both paired-device reloads and the official desktop login.
    routes.push({ kind: 'exact', path: PAIR_PATHS.root, handler: handleRootRequest })
  }
  // The read-only file viewer rides this family so it shares one LAN fence and
  // one pairing policy; the exact paths are its own (see file-viewer.ts).
  if (deps.viewer !== undefined) {
    routes.push(...makeViewerRoutes({
      service,
      lanFence,
      requirePairingForLan: deps.requirePairingForLan,
      roots: deps.viewer.roots,
      ctx: deps.viewer.ctx,
    }))
  }
  return routes
